...
{
    RGBTRIPLE image_replace[height][width];
    int i = 0;
    int j = 0;
    while (i < height)
    {
        ...
        while (j < width)
        {
            ...
            image_replace[i][j].rgbtRed = round(sum_red / total_pixels);
            image_replace[i][j].rgbtGreen = round(sum_green / total_pixels);
            image_replace[i][j].rgbtBlue = round(sum_blue / total_pixels);
            j++;
        }
        i++;
    }
